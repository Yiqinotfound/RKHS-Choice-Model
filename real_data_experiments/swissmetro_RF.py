...
def cross_validate(dataset: SwissMetroDataset, model_args: dict):
    ...
    for train_idx, val_idx in kf.split(temp_idx):
        ...
        data_rows = [
            [fold]+ list(model_args.values()) + [model.nll_train, model.nll_val, model.nll_test, model.acc_train, model.acc_val, model.acc_test]
        ] 
        header_columns = ["fold"] + list(model_args.keys()) + ["nll_train", "nll_val", "nll_test", "acc_train", "acc_val", "acc_test"]
